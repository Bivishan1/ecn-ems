...
const [selectedEmployeeLoading, setSelectedEmployeeLoading] = useState(false);
  ...
  try {
    ...
    console.error("Fetch employee records error:", err.response?.data || err.message);
  ...
  try {
    ...
    window.URL.revokeObjectURL(url);
        ...
        console.error("Export employee records error:", err.response?.data || err.message);
...
// Admin view selected office records 
const viewOfficeEmployees = async (officeItem) => {
  try {
    ...
    const res = await axiosInstance.get(
      `/employee/admin/office/${officeItem.office._id}`
    );
    ...
    showToast(
      "error",
      err.response?.data?.message || "Failed to fetch office employee records"
    );
  } finally {
        ...
        {employeeLoading ? (
          ...
          employees.map((employee) => (
            <tr key={employee._id} className="border-b">
              <td className="py-3 pr-4">
                {employee.office?.officeCode} - {employee.office?.officeName}
              </td>
        ...
        {submittedOffices.length === 0 ? (
          ...
          submittedOffices.map((item) => (
            <tr key={item.submissionId} className="border-b">
              ...
              <td className="py-3 pr-4">{item.submittedByName || "N/A"}</td>

              <td className="py-3 pr-4">{item.submittedByContact || "N/A"}</td>
  ...
  <div className="bg-white rounded-2xl shadow p-6">
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Admin can review, edit, delete, and export this office’s records.
        </p>
      </div>
          ...
          {selectedEmployeeLoading ? (
            ...
            selectedEmployees.map((employee) => (
              <tr key={employee._id} className="border-b">
                ...
                <td className="py-3 pr-4">{employee.position || "N/A"}</td>
